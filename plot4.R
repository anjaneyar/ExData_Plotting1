...
gap <- as.numeric(s2$Global_active_power)
vol <- as.numeric(s2$Voltage)
rap <- as.numeric(s2$Global_reactive_power)

plot(x=c2,y=gap,type="l",xlab="",ylab="Global Active Power")

plot(x=c2,y=vol,type="l",xlab="datetime",ylab="Voltage")
...
plot(x=c2,y=rap,type="l",xlab="datetime",ylab="Global_reactive_power")
